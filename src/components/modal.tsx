...
const Modal: React.FC<ModalProps> = ({ children, open, onClose }) => {
    ...
    return ReactDOM.createPortal(
        <div onClick={onClose} className="fixed inset-0 flex justify-center items-center transition-colors bg-black/20 z-50">
            <div className="w-6/12 h-4/6 bg-brand-400 p-4 max-sm:w-9/12 max-sm:h-4/5 relative" onClick={(e) => e.stopPropagation()}>
                <button className="absolute top-2 right-2 text-2xl bg-brand-300 hover:bg-brand-100 px-2 " onClick={onClose}>X</button>
                <h2 className="font-semibold text-brand-100 text-2xl max-sm:text-lg">Horarios disponibles</h2>
